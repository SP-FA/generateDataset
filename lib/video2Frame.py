...
class Video2Frame:
    ...
    def getFrames(self, args=None, genTXT:bool=False, lmt:int=None, names:list=[]) -> None:
        '''
        对每个视频创建文件夹，保存 frame
        PARAMETER:
        ...
        for i in range(begin, end):
            self.getFrameFromVideo(i, genTXT, lmt, names)

    def getFrameFromVideo(self, i:int=0, genTXT:bool=False, lmt:int=None, names:list=[]) -> None:
        '''
        对某个视频创建文件夹，保存 frame
        PARAMETER:
         @ i: 第i个视频，负数表示倒数第几
